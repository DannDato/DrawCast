...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if (!modifier && key === 'p') { setTool('draw'); return; }
      if (!modifier && key === 'e') { setTool('eraser'); return; }
      if (modifier && key === 'g') {
        event.preventDefault();
  ...
  return (
    <div className="dc-editor">
      <Toolbar tool={tool} setTool={setTool} guide={guide} setGuide={setGuide} onClear={clear} onUndo={undo} onRedo={redo} onCopy={() => copySelection()} onCut={() => cutSelection()} onPaste={() => pasteClipboard()} canUndo={history.past.length > 0} canRedo={history.future.length > 0} canCopy={selectedIds.length > 0} canPaste={Boolean(clipboardPayload?.objects?.length)} connected={connected} />
