...
import { getChannelState, setObject, removeObject, clearObjects, connectRole, disconnectRole } from '../services/channelRuntimeService.js';
...
export function configureSockets(io) {
  io.on('connection', (socket) => {
    ...
    const edit=(event,handler)=>socket.on(event,(payload)=>{ if(!joined||joined.role!=='editor')return socket.emit('access-denied'); handler(joined.channelId,payload); });
