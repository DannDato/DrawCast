...
router.use(verifyToken, asyncHandler(requireChannelEditor));
...
router.param('slot', (req, res, next, value) => {
  ...
  if (!req.guideSlot) return res.status(400).json({ message: 'Elige una de las tres guías del lienzo.' });
  next();
});

router.get('/:slot', authReadLimiter, asyncHandler(async (req, res) => {
...
router.put('/:slot', mutationLimiter, asyncHandler(async (req, res) => {
