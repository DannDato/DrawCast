...
export default function Profile() {
  ...
  return <div className="mx-auto w-[min(1200px,calc(100%-32px))] py-8 pt-7">
    <div className="mb-[18px] flex flex-col items-start justify-between gap-[18px] md:flex-row md:items-end [&_h1]:my-1 [&_h1]:text-[clamp(32px,5vw,48px)] [&_h1]:leading-none [&_p]:m-0">
      ...
      <button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 bg-[#101216]  hover:bg-[#171a20]" onClick={() => loadProfile({ force: true })}><RefreshCw size={16} /> Actualizar</button>
    </div>

    {notice && <div className={`mb-4 flex items-center gap-2  border border-[var(--dc-accent)] px-3.5 py-3 font-semibold ${notice.type === 'error' ? 'bg-[#171a20]' : 'bg-[#101216]'}`}><span>{notice.type === 'success' ? <Check size={17} /> : <X size={17} />}</span>{notice.text}</div>}
    ...
    <div className="grid grid-cols-1 items-start gap-[18px] md:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="grid gap-[18px] md:sticky md:top-[84px]">
        <section className=" bg-black/20 bg-[#101216] p-5 shadow-[0_8px_24px_rgba(0,0,0,0.06)] flex flex-col items-center text-center [&_h2]:mb-0.5 [&_h2]:mt-[15px] [&_h2]:text-[22px]">
          ...
          {/* <div className="my-3 flex flex-wrap justify-center gap-1.5"><span className="inline-flex rounded-full bg-black/20 bg-[#171a20] px-2 py-1 text-xs">{shown?.role?.name || shown?.role?.key || 'Sin rol'}</span><span className="inline-flex rounded-full bg-black/20 bg-[#171a20] px-2 py-1 text-xs">{shown?.status || 'Sin estado'}</span></div> */}
          <div className="mb-2 mt-1 flex items-center gap-2"><button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 bg-[#101216]  hover:bg-[#171a20]" onClick={() => avatarInput.current?.click()} disabled={busy === 'avatar'}><Camera size={16} /> {shown?.avatarUrl ? 'Cambiar foto' : 'Subir foto'}</button>{shown?.avatarUrl && <button className="inline-grid h-9 w-9 place-items-center  bg-black/20 bg-[#101216]  transition hover:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:opacity-50" onClick={deleteAvatar} disabled={busy === 'avatar'} title="Eliminar foto"><Trash2 size={16} /></button>}</div>
          <small className="">JPG, PNG o WEBP · máximo 5 MB</small>
        </section>

        <section className=" bg-black/20 bg-[#101216] p-5 shadow-[0_8px_24px_rgba(0,0,0,0.06)] grid gap-3.5">
      ...
      <div className="grid gap-[18px]">
        <section className=" bg-black/20 bg-[#101216] p-5 shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
          ...
          <div className="grid grid-cols-1 gap-3.5 md:grid-cols-2"><label className="grid gap-1.5 text-sm font-bold">Nombre para mostrar<input className="w-full  border border-[#353942] bg-[#101216] px-3 py-[11px] text-[#ebebeb] outline-none transition focus:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:bg-[#171a20] disabled:" value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={120} /></label>
            <label
              className="grid gap-1.5 text-sm font-bold">Correo electrónico<input className="w-full  border border-[#353942] bg-[#101216] px-3 py-[11px] text-[#ebebeb] outline-none transition focus:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:bg-[#171a20] disabled:" value={shown?.email || ''} disabled /></label><label className="grid gap-1.5 text-sm font-bold">Nombre de usuario<input className="w-full  border border-[#353942] bg-[#101216] px-3 py-[11px] text-[#ebebeb] outline-none transition focus:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:bg-[#171a20] disabled:" value={shown?.username || ''} disabled /></label>
              
          </div>
          <div className="mt-[18px] flex flex-col items-stretch justify-between gap-4 border-t border-[#2a2e37] pt-4 md:flex-row md:items-center"><span className="">El nombre de usuario no se cambia aquí. Si quieres cambiar tu correo, hazlo en la sección de abajo.</span><button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50" onClick={saveProfile} disabled={busy === 'profile'}>{busy === 'profile' ? 'Guardando…' : 'Guardar cambios'}</button></div>
        </section>

        <section className=" bg-black/20 bg-[#101216] p-5 shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
          <div className="mb-4"><strong className="text-base">Correo y métodos de acceso</strong><p className="mt-1 text-sm text-[#7e8592]">Cambia tu correo con un código de verificación y administra las cuentas que usas para entrar.</p></div>
          <div className="grid gap-3">
            <label className="grid gap-1.5 text-sm font-bold">Nuevo correo<input className="w-full  border border-[#353942] bg-[#101216] px-3 py-[11px] text-[#ebebeb] outline-none focus:border-[var(--dc-accent)]" type="email" value={newEmail} onChange={(event) => setNewEmail(event.target.value)} /></label>
            {hasPassword && <label className="grid gap-1.5 text-sm font-bold">Contraseña actual<input className="w-full  border border-[#353942] bg-[#101216] px-3 py-[11px] text-[#ebebeb] outline-none focus:border-[var(--dc-accent)]" type="password" value={emailPassword} onChange={(event) => setEmailPassword(event.target.value)} /></label>}
            {!emailChallengeId ? <button className="inline-flex w-fit items-center justify-center  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white" onClick={requestEmailChange} disabled={!newEmail || busy === 'email'}>Enviar código al nuevo correo</button> : <div className="flex flex-wrap gap-2"><input className="w-44  border border-[#353942] bg-[#101216] px-3 py-[11px] text-[#ebebeb] outline-none focus:border-[var(--dc-accent)]" inputMode="numeric" maxLength={6} placeholder="Código de 6 dígitos" value={emailCode} onChange={(event) => setEmailCode(event.target.value.replace(/\D/g, ''))} /><button className=" border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white" onClick={confirmEmailChange} disabled={emailCode.length !== 6 || busy === 'email-confirm'}>Confirmar correo</button></div>}
          </div>
          <div className="mt-5 border-t border-[#2a2e37] pt-4">
            {connectedAccounts.some((account) => account.provider === 'google') ? <div className="flex flex-wrap items-center justify-between gap-3"><div><strong className="block">Google conectado</strong><span className="text-sm text-[#7e8592]">{connectedAccounts.find((account) => account.provider === 'google')?.email}</span></div><button className=" border border-[var(--dc-accent)] bg-[#101216] px-3.5 py-2.5 text-[#ebebeb] disabled:opacity-50" onClick={disconnectGoogle} disabled={!hasPassword || busy === 'google'}>{hasPassword ? 'Desconectar Google' : 'Configura una contraseña para desconectar'}</button></div> : <div><strong className="mb-2 block">Conectar Google</strong><GoogleConnectButton onConnected={handleGoogleConnected} onError={handleGoogleError} /></div>}
          </div>
        </section>
        ...
        <section className=" bg-black/20 bg-[#101216] p-5 shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
          <div className="mb-[18px] flex items-start gap-2.5 [&>svg]:mt-px [&>svg]:min-w-5 [&>div]:grid [&>div]:min-w-0 [&>div]:gap-1 [&_strong]:text-base [&_span]:text-[13px] [&_span]: flex-wrap items-start md:items-center [&_.btn]:md:ml-auto [&_.btn]:whitespace-nowrap"><Monitor size={20} /><div><strong>Dispositivos y sesiones</strong><span>Revisa dónde está abierta tu cuenta y cierra accesos individualmente.</span></div>
            <button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[#101216] px-3.5 py-2.5 text-[#ebebeb] transition hover:bg-[#171a20] disabled:cursor-not-allowed disabled:opacity-50" onClick={revokeOthers} disabled={busy === 'others' || sessions.filter((row) => !row.current).length === 0}>Cerrar las demás</button>
          </div>
          <div className="grid gap-2.5">
            {sessions.length === 0 && <div className=" bg-black/20 bg-[#171a20] p-4 ">No hay sesiones activas para mostrar.</div>}
            {sessions.map((session) => {
              const device = deviceInfo(session.userAgent);
              const DeviceIcon = device.mobile ? Smartphone : Laptop;
              return <div className={`grid grid-cols-[42px_minmax(0,1fr)] items-center gap-3  border p-3 md:grid-cols-[42px_minmax(0,1fr)_auto] [&>button]:col-span-full md:[&>button]:col-span-1 ${session.current ? 'border-[var(--dc-accent)] bg-[#171a20]' : 'border-[#2a2e37] bg-[#101216]'}`} key={session.id}>
                <div className="grid h-[42px] w-[42px] place-items-center  bg-black/20 bg-[#101216]"><DeviceIcon size={21} /></div>
                <div className="grid min-w-0 gap-1 [&>div]:flex [&>div]:flex-wrap [&>div]:items-center [&>div]:gap-2 [&>span]:break-all [&>span]:text-[13px] [&>span]: [&>small]:break-all [&>small]:text-[11px] [&>small]:"><div><strong>{device.title}</strong>{session.current && <span className="inline-flex rounded-full bg-black/20 bg-[#171a20] px-2 py-1 text-xs">Este dispositivo</span>}</div><span>{session.ip || 'IP no disponible'} · Inicio {formatDate(session.createdAt)}</span><small>Expira {formatDate(session.expiresAt)}</small></div>
                <button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 border-[#2a2e37] bg-transparent  hover:border-[var(--dc-accent)] hover:bg-[#171a20]" onClick={() => revokeSession(session)} disabled={busy === `session:${session.id}`}><LogOut size={16} /> Cerrar</button>
              </div>;
            })}
          </div>
        </section>
        ...
        <section className=" bg-black/20 bg-[#101216] p-5 shadow-[0_8px_24px_rgba(0,0,0,0.06)] ">
          ...
          <div className="flex flex-wrap gap-2"><button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 bg-[#101216]  hover:bg-[#171a20]" onClick={logoutCurrent} disabled={busy === 'logout'}>Cerrar esta sesión</button><button className="inline-flex items-center justify-center gap-2  border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3.5 py-2.5 text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50" onClick={logoutAll} disabled={busy === 'all'}>Cerrar en todos los dispositivos</button></div>
        </section>
      </div>
    </div>
  </div>;
