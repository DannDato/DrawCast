    ...
    <div className="grid grid-cols-1 items-stretch gap-[18px] xl:grid-cols-[300px_minmax(0,1fr)_300px]">
      <section className="flex min-h-[440px] h-full flex-col items-center justify-center bg-[var(--dc-panel)] p-5 text-center shadow-[0_8px_24px_var(--dc-shadow-soft)]">
        <div className="relative h-[150px] w-[150px]">
          {shown?.avatarUrl ? <img className="h-[150px] w-[150px] rounded-full border border-[var(--dc-accent)] bg-[var(--dc-button-secondary-hover)] object-cover" src={shown.avatarUrl} alt="Foto de perfil" /> : <div className="grid h-[150px] w-[150px] place-items-center rounded-full border border-[var(--dc-accent)] bg-[var(--dc-button-secondary-hover)] text-[48px] font-black">{initial}</div>}
      ...
      <section className="min-h-[440px] h-full bg-[var(--dc-panel)] p-5 shadow-[0_8px_24px_var(--dc-shadow-soft)]">
        <div className="mb-[22px] flex justify-end"><h1 className="m-0 flex flex-wrap justify-end gap-x-2 font-['Bebas_Neue'] text-[2rem] font-normal uppercase leading-none tracking-[.02em] text-[var(--dc-text)] max-[680px]:text-[1.75rem]"><span>TU</span><span className="text-[var(--dc-accent)]">PERFIL</span></h1></div>
        <div className="grid gap-3.5">
          <label className="grid gap-1.5 text-sm font-bold">Nombre para mostrar<input className="w-full border border-[var(--dc-input-border)] bg-[var(--dc-button-secondary-bg)] px-3 py-[11px] text-[var(--dc-text)] outline-none transition focus:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:bg-[var(--dc-surface-hover)] disabled:text-[var(--dc-text-disabled)]" value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={120} /></label>
          <label className="grid gap-1.5 text-sm font-bold">Correo electrónico<input className="w-full border border-[var(--dc-input-border)] bg-[var(--dc-button-secondary-bg)] px-3 py-[11px] text-[var(--dc-text)] outline-none transition focus:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:bg-[var(--dc-surface-hover)] disabled:text-[var(--dc-text-disabled)]" value={shown?.email || ''} disabled /></label>
          <label className="grid gap-1.5 text-sm font-bold">Nombre de usuario<input className="w-full border border-[var(--dc-input-border)] bg-[var(--dc-button-secondary-bg)] px-3 py-[11px] text-[var(--dc-text)] outline-none transition focus:border-[var(--dc-accent)] disabled:cursor-not-allowed disabled:bg-[var(--dc-surface-hover)] disabled:text-[var(--dc-text-disabled)]" value={shown?.username || ''} disabled /></label>
        </div>
        ...
        <section className="flex min-h-[330px] flex-col bg-[var(--dc-panel)] p-5 shadow-[0_8px_24px_var(--dc-shadow-soft)]">
          ...
          <div className="flex flex-1 items-center justify-center"><div className="flex flex-wrap justify-center gap-2">
            {accessProviders.map((provider) => {
              ...
              const cardClass = `grid h-[56px] w-[56px] place-items-center border border-[var(--dc-input-border)] bg-[var(--dc-button-secondary-bg)] p-2.5 transition ${connected ? 'shadow-[0_0_18px_var(--dc-shadow-soft)] hover:border-[var(--dc-accent)]' : 'grayscale opacity-55 hover:opacity-90'} disabled:cursor-not-allowed disabled:opacity-50`;
