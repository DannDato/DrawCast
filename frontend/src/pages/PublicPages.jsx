...
const pages = {
    privacy: {
        ...
        title: "POLÍTICA DE",
        accent: "PRIVACIDAD",
        description: "Cómo tratamos la información necesaria para operar DrawCast.",
        sections: [
            ...
            ["Conservación y control", [
                ...
                "Puedes actualizar datos de cuenta desde DrawCast y solicitar atención sobre privacidad mediante el canal de soporte publicado en este sitio.",
            ]],
        ],
    },
    terms: {
        ...
        title: "TÉRMINOS Y",
        accent: "CONDICIONES",
        description: "Reglas básicas para utilizar DrawCast y sus espacios colaborativos.",
        sections: [
            ["Cuenta y acceso", [
            ...
            ["Uso permitido", [
                "No debes utilizar DrawCast para vulnerar sistemas, distribuir contenido ilegal, abusar de otros usuarios o interferir deliberadamente con la disponibilidad del servicio.",
            ...
            ["Tu contenido", [
                "Conservas la responsabilidad sobre los textos, imágenes, diseños y demás contenido que cargas o produces en DrawCast.",
            ...
            ["Disponibilidad y cambios", [
                "DrawCast puede evolucionar, modificar funciones o establecer límites técnicos para mantener seguridad y estabilidad.",
    ...
    cookies: {
        ...
        title: "POLÍTICA DE",
        accent: "COOKIES",
        description: "Qué cookies utiliza DrawCast y para qué sirven.",
        sections: [
            ["Cookies necesarias", [
                "DrawCast utiliza cookies necesarias para mantener una sesión autenticada de forma segura.",
            ...
            ["Preferencias y almacenamiento", [
                "Las preferencias propias de DrawCast se asocian a tu cuenta cuando corresponde. El navegador también puede conservar información técnica estrictamente necesaria para completar ciertos flujos.",
            ]],
            ["Servicios externos", [
            ...
            ["Cambios futuros", [
                "Si DrawCast incorpora cookies no esenciales, analítica o publicidad que requieran controles adicionales, esta política y la experiencia de consentimiento se actualizarán antes de utilizarlas cuando corresponda.",
            ]],
        ],
    },
    faq: {
        ...
        title: "PREGUNTAS",
        accent: "FRECUENTES",
        description: "Respuestas rápidas sobre el flujo principal de DrawCast.",
        sections: [
            ["¿Qué es DrawCast?", ["Es una herramienta para construir y operar overlays colaborativos en tiempo real y proyectarlos en software de streaming mediante una Browser Source."]],
    ...
    security: {
        ...
        sections: [
            ...
            ["Enlaces y UUID", [
                "DrawCast utiliza identificadores públicos no incrementales para evitar exponer IDs internos de base de datos.",
            ...
            ["Reportar un problema", [
                supportEmail
                    ...
                    : "Si detectas una posible vulnerabilidad, evita publicarla y utiliza el canal de soporte oficial que DrawCast publique para reportarla de forma responsable.",
            ]],
        ],
    },
};
    ...
    if (isLegal) return (
        <main className="min-h-screen bg-white text-black">
            <div className="mx-auto w-full max-w-[1100px] px-6 md:px-10 xl:px-0">
                <Helmet>
                    <title>{`${data.accent} — DrawCast`}</title>
                    <meta name="description" content={data.description} />
                </Helmet>

                <nav className="flex items-center justify-between gap-5 border-b border-black/10 py-5">
                    <Link className="font-['Bebas_Neue'] text-2xl font-normal tracking-[0.06em] text-black" to="/">DRAWCAST // OVERLAY</Link>
                    <div className="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.08em]">
                        <Link className="border border-black/15 px-3 py-2 text-black transition hover:border-black" to="/">Inicio</Link>
                        <Link className="border border-black bg-black px-3 py-2 text-white transition hover:bg-black/80" to="/login">Entrar</Link>
                    </div>
                </nav>

                <header className="py-12 md:py-16">
                    <span className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-black/45">{data.eyebrow}</span>
                    <h1 className="mt-4 font-['Bebas_Neue'] text-[clamp(3rem,7vw,5.5rem)] font-normal uppercase leading-[0.86] tracking-[-0.01em] text-black">{data.title} {data.accent}</h1>
                ...
                <div className="border-t border-black/10 pb-14">
                    {data.sections.map(([title, paragraphs], index) => (
                        <section key={title} className="grid gap-4 border-b border-black/10 py-8 md:grid-cols-[210px_minmax(0,1fr)] md:gap-10">
                            <h2 className="m-0 text-base font-bold uppercase tracking-[0.04em] text-black"><span className="mr-2 font-mono text-xs text-black/45">{String(index + 1).padStart(2, "0")}</span>{title}</h2>
                ...
                <div className="mx-auto w-full max-w-[1240px] px-6 md:px-10 xl:px-0">
                    <nav className="flex items-center justify-between gap-5 border-b border-[var(--dc-text)]/15 py-5">
                        <Link className="font-['Bebas_Neue'] text-2xl font-normal tracking-[0.06em]" to="/">
                            DRAWCAST <span className="text-[var(--dc-line)]">//</span> <span className="text-[var(--dc-accent)]">OVERLAY</span>
                        </Link>
                        <div className="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.08em]">
                            <Link className="border border-[var(--dc-text)]/25 px-3 py-2 transition hover:border-[var(--dc-accent)] hover:text-[var(--dc-accent)]" to="/">Inicio</Link>
                            <Link className="border border-[var(--dc-accent)] bg-[var(--dc-accent)] px-3 py-2 text-white" to="/login">Entrar</Link>
                        </div>
                    </nav>
                    ...
                    <header className="py-16 md:py-24">
                        ...
                        <h1 className="mt-5 font-['Bebas_Neue'] text-[clamp(4rem,12vw,8rem)] font-normal uppercase leading-[0.78] tracking-[-0.02em]">
                            {data.title} <span className="text-[var(--dc-accent)]">{data.accent}</span>
                        </h1>
                        ...
                        {data.sections.map(([title, paragraphs], index) => (
                            <section key={title} className="grid gap-5 border-b border-[var(--dc-text)]/10 py-10 md:grid-cols-[220px_minmax(0,1fr)] md:gap-10">
                                <div className="font-['Bebas_Neue'] text-3xl uppercase text-[var(--dc-text-muted)]">
                                    <span className="mr-2 text-[var(--dc-accent)]">{String(index + 1).padStart(2, "0")}</span>{title}
                                </div>
