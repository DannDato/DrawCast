...
export default function EditorPreferencesSettings({ value, onChange, onSave, onReset, saving }) {
  ...
  return <div className="grid gap-[18px]">
    <div className="flex flex-col items-start justify-between gap-3 border border-[var(--dc-line)] bg-[var(--dc-surface-raised)] p-4 md:flex-row md:items-center"><div><strong className="block">Predeterminados de herramientas</strong><span className="text-sm text-[var(--dc-text-muted)]">Se aplican al crear elementos nuevos. Nunca se guardan dentro de un diseño ni modifican capas que ya existen.</span></div><div className="flex shrink-0 flex-wrap gap-2"><button className="inline-flex items-center gap-2 border border-[var(--dc-button-secondary-border)] bg-[var(--dc-button-secondary-bg)] px-3.5 py-2.5 text-sm font-bold" onClick={onReset} disabled={saving}><RotateCcw size={15} /> Restaurar</button><button className="inline-flex items-center gap-2 border border-[var(--dc-button-primary-border)] bg-[var(--dc-button-primary-bg)] px-3.5 py-2.5 text-sm font-bold text-[var(--dc-button-primary-text)] disabled:opacity-50" onClick={onSave} disabled={saving}><Save size={15} /> {saving ? 'Guardando…' : 'Guardar configuración'}</button></div></div>
