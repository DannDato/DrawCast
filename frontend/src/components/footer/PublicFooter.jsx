...
function FooterColumn({ title, links, legal }) {
    return (
        <div>
            <h3 className={legal ? "mb-4 font-['Bebas_Neue'] text-2xl font-normal uppercase tracking-[0.03em] text-white" : "mb-4 font-['Bebas_Neue'] text-2xl font-normal uppercase tracking-[0.03em] text-[var(--dc-text)]"}>{title}</h3>
            <div className={legal ? "grid gap-2.5 font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-white/55" : "grid gap-2.5 font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--dc-text)]/45"}>
                {links.map((item) => (
                    <Link key={item.to} className={legal ? "w-fit transition hover:text-[var(--dc-accent-four)] hover:underline" : "w-fit transition hover:text-[var(--dc-accent-four)]"} to={item.to}>
                        {item.label}
                    </Link>
                ))}
            </div>
        ...
        <footer className={legal ? "border-t border-black/10 bg-[#242424] text-white" : "border-t border-[var(--dc-text)]/15 bg-[#101010]"}>
            <div className={legal ? "mx-auto w-full max-w-[1100px] px-6 py-10 md:px-10 md:py-12 xl:px-0" : "mx-auto w-full max-w-[1240px] px-6 py-14 md:px-10 md:py-16 xl:px-0"}>
                <div className={legal ? "grid gap-8 border-b border-white/10 pb-8 md:grid-cols-[1.4fr_repeat(3,1fr)]" : "grid gap-10 border-b border-[var(--dc-text)]/10 pb-12 md:grid-cols-[1.4fr_repeat(3,1fr)]"}>
                    <div className="max-w-[360px]">
                        ...
                        <div className="mt-5 flex flex-wrap gap-2">
                            {socialLinks.map((item) => {
                                return (
                                    <a key={item.label} className={legal ? "grid h-9 w-9 place-items-center border border-white/15 text-white transition hover:border-[var(--dc-accent-three)]" : "grid h-9 w-9 place-items-center border border-[var(--dc-text)]/15 text-[var(--dc-text)]/45 transition hover:border-[var(--dc-accent-three)] hover:text-[var(--dc-accent-four)]"} href={item.href} target="_blank" rel="noopener noreferrer" aria-label={item.label} title={item.label}>
                    ...
                    <div>
                        <h3 className={legal ? "mb-4 font-['Bebas_Neue'] text-2xl font-normal uppercase tracking-[0.03em] text-white" : "mb-4 font-['Bebas_Neue'] text-2xl font-normal uppercase tracking-[0.03em] text-[var(--dc-text)]"}>Legal</h3>
                        <div className={legal ? "grid gap-2.5 font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-white/55" : "grid gap-2.5 font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--dc-text)]/45"}>
                            ...
                            {supportEmail && (
                                <a className={legal ? "mt-2 inline-flex w-fit items-center gap-2 transition hover:text-[var(--dc-accent-four)] hover:underline" : "mt-2 inline-flex w-fit items-center gap-2 transition hover:text-[var(--dc-accent-four)]"} href={`mailto:${supportEmail}`}>
